...
export default function Example() {
  ...
  const [errConfirmPwd, setErrConfirmPwd] = useState(null);
  // const [error, setError] = useState("");
  ...
  const handleSave = async (e) => {
    ...
    const res = await axios.put(
      "ProfileController/update_password/" + id,
      JSON.stringify(data),
      {
        headers: {
    ...
    if (res.status === 201) {
      ...
      setSuccess("");
      const newErr = {
        old_password: res.data.errors.old_password,
        new_password: res.data.errors.password,
